...
class NetworkingService {
    ...
    func getCitiesToShow() -> [Location] {
        return  [Location(placeName: "London", latitude: 51.5072, longitude: 0.1276),
                 ...
                 Location(placeName: "Brussels", latitude: 50.8476, longitude: 4.357),
    ...
    func getCurrentWeahter(_ location: Location, completion: @escaping ((Weather?) -> Void)) {
    ...
    func getForecase(_ location: Location, completion: @escaping ((ForecastModel?) -> Void)) {
        guard var urlComps = URLComponents(string: Endpoints.getForecase.stringValue) else { return }
        let queryItems = [URLQueryItem(name: "lat", value: "\(location.latitude)"), URLQueryItem(name: "lon", value: "\(location.longitude)"), URLQueryItem(name: "appid", value: "\(Self.apiKey)")]
        urlComps.queryItems = queryItems
        guard let url = urlComps.url else { return }
         let task = session.dataTask(with: url) { data, response, error in
             guard let data = data, error == nil else { return }
             do {
                 let res = try JSONDecoder().decode(ForecastModel.self, from: data)
                 completion(res)
             } catch let error as NSError {
                 completion(nil)
                 print("Failed to load: \(error.localizedDescription)")
